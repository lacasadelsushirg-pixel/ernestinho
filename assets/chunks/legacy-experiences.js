...
true && (React.createElement("section", { className: "py-12 bg-white" },
                React.createElement("div", { className: "max-w-7xl mx-auto px-4 sm:px-6" },
                    ...
                    React.createElement("div", { className: "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-7" }, (window.ACTIVIDADES_NUEVAS || []).map((actividad) => React.createElement("article", { key: actividad.id, className: "bg-slate-50 rounded-3xl overflow-hidden border border-slate-100  flex flex-col" },
            ...
            seccionActual.startsWith('actividad_') && React.createElement(ActividadNuevaDetalle, { actividad: (window.ACTIVIDADES_NUEVAS || []).find(a => `actividad_${a.id}` === seccionActual), onVolver: onBack, lang: lang }),
