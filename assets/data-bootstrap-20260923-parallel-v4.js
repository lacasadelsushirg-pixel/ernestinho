...
(async()=>{
 ...
 if(root&&!root.childNodes.length)root.innerHTML='<div style="min-height:70vh;display:flex;align-items:center;justify-content:center;padding:24px;font:600 18px Arial,sans-serif;color:#0b2528;text-align:center">Preparando tu guía de Río…</div>';
 ...
 if(lang!=='es'){await window.__ecEnsureLanguagePayloads();
  try{const parts=await Promise.all(Array.from({length:23},(_,i)=>fetch('/assets/translations/heavy-'+String(i+1).padStart(2,'0')+'.part?v=20260923',{cache:'force-cache'}).then(r=>{if(!r.ok)throw Error(r.url+' '+r.status);return r.text()})));(0,eval)(parts.join(''));await load('/assets/translations/heavy-tail.js?v=20260923-full-parse1');}catch(e){console.error('EC heavy translations',e);}
 }
 ...
 await Promise.all(urls.map(url=>window.__ecLoadData(url).catch(e=>console.error('EC data load failed',url,e)))); try{const m=await fetch('/data/recorridos/manifest.json',{cache:'force-cache'}).then(r=>r.json());await Promise.all((m.files||[]).map(url=>window.__ecLoadData(url).catch(e=>console.error('EC route data load failed',url,e))))}catch(e){console.error('EC recorridos load failed',e)}
 const x=document.createElement('script');x.src='/assets/app-runtime-20260923-validated-v3.js?v=3';x.onerror=()=>console.error('EC app load failed');document.body.appendChild(x);
