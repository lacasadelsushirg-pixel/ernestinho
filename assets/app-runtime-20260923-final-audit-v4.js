...
const __ecGastronomiaData = () => [...(window.GASTRONOMIA_BASE||[]), ...(window.GASTRONOMIA_EXTRA||[])].map((item, index) => ({...item,img:item.img||FOTOS_GASTRONOMIA[index % Math.max(FOTOS_GASTRONOMIA.length,1)],mapa:`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(item.nombre + ' ' + item.barrio + ' Rio de Janeiro')}`,horario:'Los horarios pueden cambiar. Confirma el día de tu visita en el mapa oficial.'}));
...
const __ecActividadesNuevas = () => Array.isArray(window.ACTIVIDADES_NUEVAS) ? window.ACTIVIDADES_NUEVAS : [];
...
function ActividadNuevaDetalle(props){return React.createElement(ECLazyRoute,{src:'/assets/chunks/ActividadNuevaDetalle.js?v=20260923-modular',name:'ActividadNuevaDetalle',componentProps:props,dataUrls:['/data/explorar.json']})}
...
function VidaNocturnaCompleta(props){return React.createElement(ECLazyRoute,{src:'/assets/chunks/VidaNocturnaCompleta.js?v=20260923-modular',name:'VidaNocturnaCompleta',componentProps:props,dataUrls:['/data/vida-nocturna.json']})}
...
const __ecGuiaInfo = () => window.GUIA_INFO || {};
const __ecConsejosNuevos = () => Array.isArray(window.CONSEJOS_NUEVOS) ? window.CONSEJOS_NUEVOS : [];
const __ecArticulosConsejos = () => Array.isArray(window.ARTICULOS_CONSEJOS) ? window.ARTICULOS_CONSEJOS : [];

function GuiaRioCompleta(props){return React.createElement(ECLazyRoute,{src:'/assets/chunks/GuiaRioCompleta.js?v=20260923-modular',name:'GuiaRioCompleta',componentProps:props,dataUrls:["/data/guia.json","/data/consejos-viaje.json","/data/consejos-viaje-lazy52.json","/data/viaje-consejos-extra.json"]})}
function ConsejosErnestinho(props){return React.createElement(ECLazyRoute,{src:'/assets/chunks/ConsejosErnestinho.js?v=20260923-modular',name:'ConsejosErnestinho',componentProps:props,dataUrls:["/data/consejos-viaje.json","/data/consejos-viaje-lazy52.json","/data/viaje-consejos-extra.json","/data/viaje-consejos-extra-lazy52.json","/data/explorar-lazy.json","/data/transporte-lazy52.json"]})}
...
function seoResolveRoute(pathname) {
    ...
    if (m) {
        ...
        const restaurante = __ecGastronomiaData().find(x => seoSlug(x.nombre) === m[1]);
        if (restaurante)
            return { ...result, seccion: 'gastronomia', restaurante };
    }
    m = ruta.match(/^\/articulos\/([^/]+)$/);
    if (m) {
        const articulo = __ecArticulosConsejos().find(x => seoSlug(x.titulo) === m[1] || seoSlug(x.id) === m[1]);
        if (articulo)
            return { ...result, seccion: 'articulo_detalle', articulo };
    }
    m = ruta.match(/^\/guia\/([^/]+)$/);
    if (m && m[1] !== 'pix') {
        const temaId = Object.keys(__ecGuiaInfo()).find(id => { var _a; return seoSlug(((_a = __ecGuiaInfo()[id]) === null || _a === void 0 ? void 0 : _a.titulo) || id) === m[1] || seoSlug(id) === m[1]; });
        if (temaId)
            return { ...result, seccion: 'guia', temaGuia: temaId };
    }
    m = ruta.match(/^\/experiencias\/([^/]+)$/);
    if (m) {
        const all = [...EXPERIENCIAS_DATA, ...__ecActividadesNuevas()];
...
function seoCurrentPath(section, museoId, barrio, restaurante, articulo, temaGuia, consejoId) {
    ...
    if (section === 'museos' && museoId) {
        const museo = (window.MUSEOS_DATA||[]).find(x => x.id === museoId);
        const actual = seoCleanPath(window.location.pathname);
    ...
    if (section === 'guia' && temaGuia && __ecGuiaInfo()[temaGuia])
        return `/guia/${seoSlug(__ecGuiaInfo()[temaGuia].titulo || temaGuia)}`;
...
const GASTRONOMIA_INDICE_BUSQUEDA = { get(id){ const card=(window.GASTRONOMIA_NUEVAS_CARDS||[]).find(x=>x.id===id); return card?normalizarGastroNueva(`${card.title} ${(card.tags||[]).join(' ')} ${card.desc||card.descripcion||''}`):''; } };
...
function AppErnestinho(){
    ...
    const [consejoSeleccionado, setConsejoSeleccionado] = useState(rutaSeoInicial.consejoId ? (__ecConsejosNuevos().find(x => x.id === rutaSeoInicial.consejoId) || null) : null);
    ...
    useEffect(() => {
        // The previous DOM-wide translator walked every text node and attribute on mount.
        // That blocks the main thread on the large homepage. Content modules now own their
        // localized strings; keep only a lightweight hook for compatibility.
        window.__ernestinhoApplyLanguage = () => {};
        window.__ernestinhoNormCache = { pt: null, en: null };
        return () => {
            if (window.__ernestinhoApplyLanguage)
    ...
    const abrirDetalleRuta = (sec, id) => {
        ...
        if (sec === 'gastronomia')
            setRestauranteSeleccionado(__ecGastronomiaData().find(x => x.id === id) || null);
    ...
    useEffect(() => {
        const aplicarRuta = () => {
            ...
            setConsejoSeleccionado(resolved.consejoId ? (__ecConsejosNuevos().find(x => x.id === resolved.consejoId) || null) : null);
