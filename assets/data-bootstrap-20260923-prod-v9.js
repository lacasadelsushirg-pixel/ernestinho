...
(async()=>{
 ...
 try{
  ...
  await load('/assets/qh-i18n.js?v=20260923-prod-v9');
  await load('/assets/editorial-data.js?v=20260923-prod-v9');
  ...
  if(/^\/gastronomia\//.test(path)){preload.push('/data/gastronomia.json');await load('/assets/gastronomy-data.js?v=20260923-prod-v9');await Promise.race([window.__ecGastronomyReady||Promise.resolve(),new Promise(ok=>setTimeout(ok,4000))]);}
  if(/^\/museos\//.test(path)){const culture=['museos','teatros','iglesias','familia','museos-real'];await Promise.allSettled(culture.map(n=>load('/assets/culture/'+n+'.js?v=20260923-prod-v9')));}
  if(preload.length) await Promise.allSettled([...new Set(preload)].map(window.__ecLoadData));

  await load('/assets/app-runtime-20260923-prod-v9-v4.js?v=20260923-prod-v9');
  ...
  Promise.resolve().then(async()=>{
   load('/assets/gastronomy-data.js?v=20260923-prod-v9').catch(e=>console.error('EC gastronomy background',e));
   if(new URLSearchParams(location.search).get('debug')==='1') load('/assets/core-recommendation-engine.js?v=20260923-prod-v9').catch(e=>console.error('EC diagnostics engine',e));
   try{
