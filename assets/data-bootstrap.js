...
(async()=>{
 ...
 const results=await Promise.allSettled(urls.map(window.__ecLoadData));
 const failed=results.map((r,i)=>r.status==='rejected'?urls[i]:null).filter(Boolean);
 if(failed.length) console.error('EC data preload failed',failed);
 const x=document.createElement('script');x.src='/assets/app.js?v=20260922-stable5';x.onerror=()=>console.error('EC app load failed');document.body.appendChild(x);
