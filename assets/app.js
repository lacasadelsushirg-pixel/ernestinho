...
const GASTRONOMIA_INDICE_BUSQUEDA = new Map(GASTRONOMIA_NUEVAS_CARDS.map(card => [card.id, normalizarGastroNueva(`${card.title} ${(card.tags || []).join(' ')} ${textoVisibleGastroNueva(GASTRONOMIA_NUEVAS_HTML[card.id])}`)]));
