function Master4TV({ go, lang = 'es' }) {
    return React.createElement("div", { className: "min-h-screen bg-[#f7f7f4] pb-24" },
        ...
        React.createElement("section", { className: "max-w-6xl mx-auto px-4 sm:px-6 py-8" }, MASTER4_TV_REPORTS.length ? React.createElement("div", { className: "grid sm:grid-cols-2 lg:grid-cols-3 gap-5" }, MASTER4_TV_REPORTS.map(v => React.createElement("article", { key: v.title, className: "bg-white rounded-3xl border overflow-hidden" },
...
// RIO_HOJE_FALLBACK_EVENTS externalizado a JSON
